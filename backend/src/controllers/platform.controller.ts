...
export const getRunHandler=async(req:AuthenticatedRequest,res:Response)=>{const result=await pgQuery(`SELECT run_id,case_id,status,response_payload,created_at,workflow_id,workflow_version,config_version FROM orchestration_runs WHERE run_id=$1 AND tenant_id=$2`,[req.params.runId,req.user!.tenantId]);if(!result.rows[0])return res.status(404).json({error:"RUN_NOT_FOUND"});return res.json(maskPiiPayload(result.rows[0]));};
